...
void Ogre_glTF_skeletonImporter::addChidren(const std::string& skinName, const std::vector<int>& childs, Ogre::v1::OldBone* parent)
{
	...
	for (auto child : childs)
	{
		...
		std::transform(
			std::begin(node.translation),
			std::end(node.translation),
			std::begin(position),
			toFloat);

		std::transform(
			std::begin(node.rotation),
			std::end(node.rotation),
			std::begin(rotation),
			toFloat);

		bone->setPosition(Ogre::Vector3{ position.data() });
...
void Ogre_glTF_skeletonImporter::loadBoneRierarchy(const tinygltf::Skin& skin, Ogre::v1::OldBone* rootBone)
{
	...
	rootBone->setPosition(Ogre::Vector3{ position.data() });
...
Ogre::v1::SkeletonPtr Ogre_glTF_skeletonImporter::getSkeleton()
{
	...
	loadBoneRierarchy(skin, rootBone);
